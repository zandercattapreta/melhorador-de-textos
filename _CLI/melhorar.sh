...
# Script mora em _CLI/; dados (_originais/_output/_temp) ficam na RAIZ do projeto.
...
# Setup Python venv (dentro de _CLI/)
if [ ! -d "$CLI_DIR/.venv" ]; then
    log_info "Criando _CLI/.venv..."
    python3.12 -m venv "$CLI_DIR/.venv" || { log_error "Falha ao criar .venv"; exit 1; }
fi

source "$CLI_DIR/.venv/bin/activate"
# Contorno: garante o pacote no path mesmo se o .pth do editable falhar.
